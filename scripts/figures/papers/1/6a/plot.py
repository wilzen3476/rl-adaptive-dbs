...
(``artifacts/figures/papers/1/4a/checkpoint.pt``); train QAT only (10 episodes).
Train once with ``scripts/figures/papers/1/4a/plot.py``, then run this script.
...
FIGURES_DIR = Path("figures/papers/1/6a")
CACHE_DIR = Path("artifacts/figures/papers/1/6a")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Full-precision checkpoint (default: Fig 4a artifacts/figures/papers/1/4a/checkpoint.pt)",
    )
    parser.add_argument(
        "--qat-checkpoint",
        type=Path,
        ...
        if not args.skip_train:
            if args.train_fp32 and not fp32_ckpt.exists():
                ...
                print(
                    ...
                    "Run Fig 4a first: uv run python scripts/figures/papers/1/4a/plot.py "
                    f"--seed {args.seed}",
                    file=sys.stderr,
                )
                return 2
