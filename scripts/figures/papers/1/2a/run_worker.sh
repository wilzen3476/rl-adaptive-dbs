...
command_text="uv run python scripts/figures/papers/1/2a/plot.py --seeds ${SEEDS} --step-s ${STEP_S} --window-s ${WINDOW_S}"
...
set +e
taskset -c "$CPUS" uv run python scripts/figures/papers/1/2a/plot.py \
    --seeds "$SEEDS" \
    --step-s "$STEP_S" \
    --window-s "$WINDOW_S"
exit_code=$?
