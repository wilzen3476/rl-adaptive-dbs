...
(``artifacts/figures/papers/1/4a/checkpoint_skip_regular_02s.pt``). The 45 Hz
...
Run:
  ...
  uv run python -m rl_adaptive_dbs.run scripts/figures/papers/1/5a/plot.py --no-skip-regular --checkpoint artifacts/figures/papers/1/4a/checkpoint.pt --seed 1

Each run writes ``figures/papers/1/5a/efficacy_45hz_vN.png`` (N auto-increments) and updates
...
FIGURES_DIR = Path("figures/papers/1/5a")
CACHE_DIR = Path("artifacts/figures/papers/1/5a")
FIG4A_CACHE = Path("artifacts/figures/papers/1/4a")
    ...
    if args.plot_only:
        ...
        if not args.checkpoint.exists():
            ...
            if args.skip_regular:
                ...
                print(
                    "Train first: uv run python scripts/figures/papers/1/4a/plot.py --seed 1",
                    file=sys.stderr,
                )
            return 2
        payload = _run_paper_eval(
