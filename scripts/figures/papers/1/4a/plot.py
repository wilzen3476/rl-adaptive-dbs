...
Each run writes ``figures/papers/1/4a/training_beta_vN.png`` (N auto-increments),
``artifacts/figures/papers/1/4a/checkpoint.pt`` (fp32 actor for Fig 5a/6a eval),
and updates the replication image link in ``docs/figures/paper_1.md``.
...
FIGURES_DIR = Path("figures/papers/1/4a")
CACHE_DIR = Path("artifacts/figures/papers/1/4a")
...
def _vault_backed_png(path: Path) -> Path:
    ...
    Fig assets under ``figures/papers/`` are usually vault symlinks. New versioned
    PNGs must follow the same pattern so the main checkout and docs see them.
    """
    path = Path(path)
...
def update_checklist_in_doc(rows: list[tuple[str, str, str, str]]) -> None:
    """Rewrite the Fig 4a checklist Match? column in paper_1.md."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        dest="update_docs",
        action="store_false",
        help="Skip docs/figures/paper_1.md caption + replication image link",
    )
    parser.add_argument(
        "--update-checklist",
        action="store_true",
        help="Rewrite the Fig 4a side-by-side checklist in paper_1.md (off by default)",
    )
