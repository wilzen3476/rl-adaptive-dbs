...
Run:
  ...
  uv run python scripts/figures/papers/1/4b/plot.py --episodes 8 --fig4a-series artifacts/figures/papers/1/4a/series_v4.json
"""
...
FIGURES_DIR = Path("figures/papers/1/4b")
CACHE_DIR = Path("artifacts/figures/papers/1/4b")
FIG4A_CACHE_DIR = Path("artifacts/figures/papers/1/4a")
...
def _load_fig4a_payload(series_path: Path, manifest_path: Path | None) -> dict[str, Any]:
    ...
    print("Run Fig 4a first: uv run python scripts/figures/papers/1/4a/plot.py", file=sys.stderr)
    raise SystemExit(2)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        dest="update_docs",
        action="store_false",
        help="Skip docs/figures/paper_1.md updates",
    )
    parser.add_argument(
        "--update-checklist",
        action="store_true",
