...
- **Fig. 3 panel protocol (chosen):** untreated GPi $\alpha$–$\beta$ (7–35 Hz via `alpha_beta_power`) for **PD Off** (`pd=0`) vs **PD On** (`pd=1`); default **500** iterations × **100 ms** per condition; no DBS; $\theta$ is **not** drawn on the panel. Script: `scripts/figures/papers/nguyen/3/plot.py`. Tracker + gates: [figures/nguyen/replications.md](../../../figures/nguyen/replications.md).
