...
Fig. 5 compares **50 Hz** vs **30 Hz** carrier during inference; this is not a per-step RL action. **Fixed:** binary pulse/no-pulse control during training. **Chosen (adapter):** `SEA_DBSEnvAdapter.set_carrier_hz` is the eval override and **survives `reset()`** (reset used to restore `config.carrier_hz`, which silently ran Fig 5 at the Fig 4a train carrier of 130 Hz). `evaluate()` loads plant knobs (`dbs_burst_ms`, `observation_scale`, …) from the checkpoint, then writes the eval carrier into the config. **Chosen (Fig 5a eval window):** `biomarker_window_s=0.14` and `dbs_burst_ms=140` at 50 Hz (eight pulses), plus `n_obs=11` so the Eq. 4–5 mean still includes onset through step 10 (`n_obs=5` floors by step 5). A 100 ms window floors $P_\beta$ at ~0.388 after one stim, above digitized SEA steps 3–5 (~0.396 / 0.363 / 0.338); 140 ms last-window $P_\beta$ ~0.339. Amplitude and pulse-width sweeps at 50 Hz / 100 ms do not move that floor. Continuous 50 Hz on Kumaravelu bounces rather than accumulating the paper’s late slope. Fig 4a train stays 100 ms / 62 ms @ 130 Hz, $n_{\mathrm{obs}}=5$. Fig 5b stays on the checkpoint 62 ms burst and $n_{\mathrm{obs}}=5$ — a longer 30 Hz train raises $P_\beta$ further. **Chosen (Fig 5 action sampling):** hard **Gumbel-max** on actor logits (`action_mode="gumbel"`). That sample is temperature-invariant; $P(\mathrm{stim})=\mathrm{softmax}(\mathrm{logits})$. Greedy `argmax` collapses both Fig 4a actors to always-on. Fig 5a uses Gumbel offset **34** (skip-first, no late skips); Fig 5b keeps offset 1. See [docs/figures/ravivarapu/5a.md](../../figures/ravivarapu/5a.md).
