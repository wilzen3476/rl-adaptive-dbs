...
**Companion spec:** Plant dynamics and biomarker primitives — [plant.md](../../plant.md). Mehregan env ($P_\beta$ window, step timing, reward $R$, baselines) — [environment.md](../../environment.md). Benchmark suites and variant slugs — [benchmarking.md](../../benchmarking.md); CLI — [cli.md](../../cli.md). **Panel checklist (primary exit criterion):** [figures/mehregan/replications.md](../../../figures/mehregan/replications.md). Verified vs divergent details — [replication-fidelity.md](../../development/replication-fidelity.md). This document is authoritative for **policy optimization** unless the others explicitly describe the same quantity.

---

## Figure replication (scheduling)

Mehregan DDPG work is validated **panel-by-panel**, not by “Phase 3 complete.” Each training or quantization panel has a committed train+plot script under `scripts/figures/papers/mehregan/<panel>/plot.py` and qualitative gates in [figures/mehregan/replications.md](../../../figures/mehregan/replications.md).

| Panel | This spec covers | Script | Status |
|-------|------------------|--------|--------|
| Fig 4a | Training $P_\beta$ vs step; Algorithm 1 + exploration defaults | `scripts/figures/papers/mehregan/4a/plot.py` | Digitization revisit (τ 3→1.4) |
| Fig 4b | Episode reward + PSD trends | `scripts/figures/papers/mehregan/4b/plot.py` | Digitization revisit (paired live 4a series) |
...
| Fig 6b | PTQ / QAT @ 30 Hz | `scripts/figures/papers/mehregan/6b/plot.py` | **Pass** (rep v40) |
...
**Action-space mismatch (found 2026-07-07, TASK-81):** the constant-policy collapse has a deeper root cause than the reward sign bug. Our action space is a **scalar STN frequency** (`envs/mehregan/patterns.py`, `0:5:200 Hz`), whereas Mehregan et al. use a **temporal pulse pattern at a fixed mean frequency** (45/30 Hz initialization; "average frequency of action $f$" is an Algorithm 1 input). With mean rate held constant by construction, the paper's cost-free reward (Eq. 8) does **not** drive collapse — the agent shapes the *pattern*, not the energy. With our scalar-frequency action, the cost-free reward makes a single fixed frequency the correct optimum, so constant policy is the **expected** outcome, not a bug. See [environment.md](../../environment.md) §4.1 for the full finding and the A/B/C decision (accept constant / add cost term / redesign action space as fixed-budget pattern).
