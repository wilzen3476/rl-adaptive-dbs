...
| `early_sea_below_baseline` | SEA < Baseline at step 5 |
...
**Chosen (Fig 5a eval-only):** `FIG5A_INFERENCE_BURST_MS=100` (five 50 Hz pulses). Fig 4a train unchanged (62 ms @ 130 Hz). Generous early gates: MAE≤0.03 vs digitized steps 0–5, SEA drop 0→5 > 0.05, SEA below Baseline at step 5. Not 10-step magnitude polish — paper SEA step 5 is still ~0.05 below the 5-pulse floor.
...
Gumbel-max offset +2, burst **100 ms**. Baseline `[1,1,1,1,0,…]`; SEA all-1s. Steps 0–4 identical (both stim); Baseline skip at step 5 lifts it to 0.402 vs SEA 0.388. Early MAE Baseline 0.017 / SEA 0.023; SEA drop 0→5 = 0.073. All shape + early gates **pass**. Paper SEA at step 5 is still 0.338 vs our 0.388 floor. PNG: `figures/ravivarapu/images/5a/inference_50hz_v10.png`.
