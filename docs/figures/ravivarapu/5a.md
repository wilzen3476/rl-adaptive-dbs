...
Each `env.step()` **restarts** Kumaravelu from the same ICs, so last-window $P_\beta$ cannot accumulate. The eval window is a longer independent shot, not a continuous trajectory. Continuous 50 Hz (1.5–2 s, overlapping hops, trailing windows) **bounces** 0.33–0.50 instead of the paper’s late slope. **Chosen (Fig 5a $n_{\mathrm{obs}}$):** `n_obs=10` so onset ages out at step 10 and SEA can reach the 150 ms floor (~0.328). `n_obs=11` holds SEA ~0.02 above that floor; `n_obs=5` is all-floor by step 5 (v12 horizontal). Table I leaves $n_{\mathrm{obs}}$ open; Fig 4a train stays 5.
