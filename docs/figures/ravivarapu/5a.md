...
**Chosen (Fig 5a eval-only):** `FIG5A_INFERENCE_BURST_MS=100` (five 50 Hz pulses). Fig 4a train unchanged (62 ms @ 130 Hz). Generous early gates: MAE≤0.03 vs digitized steps 0–5, SEA drop 0→5 > 0.05, SEA below Baseline at **every** step 1–5. Offset +1 is required for that ordering (offset +2 overlays 0–4). Not 10-step magnitude polish — paper SEA step 5 is still ~0.05 below the 5-pulse floor.

Each `env.step()` **restarts** Kumaravelu from the same ICs, so last-window $P_\beta$ cannot accumulate. That is the Fig 4a biomarker-window convention; matching paper’s post-step-2 acceleration would need a different plant response at 50 Hz.
