...
| `early_mae_sea_3_5` | steps 3–5 MAE vs digitized SEA ≤ 0.015 |
...
**Gumbel eval seed:** plant reset stays seed 0. Offset **+1** starts Baseline with a skip so digitized paper ordering holds (Baseline above SEA from step 1). Offset +2 started `[1, 1, …]` and overlaid both traces through step 4. Torch seed 0 is a 6% all-stim Baseline draw.
...
**Chosen (Fig 5a eval-only):** `FIG5A_INFERENCE_WINDOW_S=0.14` / burst 140 ms (eight 50 Hz pulses). Fig 4a train unchanged (100 ms / 62 ms @ 130 Hz). Amplitude at 50 Hz / 100 ms does not move the ~0.388 floor. 140 ms last-window $P_\beta$ ~0.339 matches digitized SEA step 5. Early gates: MAE≤0.03 on 0–5, MAE≤0.015 on SEA 3–5, SEA below Baseline at every step 1–5. Offset +1 for that ordering.

Each `env.step()` **restarts** Kumaravelu from the same ICs, so last-window $P_\beta$ cannot accumulate. The 140 ms window is a longer independent shot, not a continuous trajectory.
