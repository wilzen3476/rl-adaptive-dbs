...
| `ptq_weight_noise` | Baseline+PTQ σ=0.03 (seed 11); SEA+PTQ σ=0.20 (seed 55), fallback 0.25 | Gaussian perturbation on a weight copy **before** `.half()`. Paper-silent; same split convention as Mehregan Fig. 6a. SEA Gumbel margins are ~8, so σ≤0.10 never leaves always-on. |
