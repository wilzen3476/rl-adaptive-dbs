...
## Locked replication (v89 train → v58 PNG, seed 0)
...
**Status:** **Pass** — `shape_pass=True`, `pass=True` (all manifest gates true at rep v58). Same locked v89 train / `series.json` as v57; v58 is a hygiene `--plot-only` promote that repaired tracker image/caption/status drift (stale v49 image link vs Pass v57). Ep 2 SEA PSD **0.456** (v40 dip **0.303** removed). Later `_vN` PNGs after v42 are overlay/legend cosmetic replots of the same locked train. Promote now rewrites the Fig 4a Status `rep vN` on every run (not only Open→Pass).
