...
## Locked replication (v90 train in flight → display roll10; prior ship v89→v58)
...
| Mid stim logit boost | ep 12–**80**, +0.4 (**v90** extended; was 12–38) | — |
| Midlate stim logit boost | ep 80–120, **+0.34** (**v90**; was 0.29) | — |
| Late no-stim ramp | ep 138–150, max **+0.22** (**v90**; was 0.40) | — |
| `polyak_tau` | 0.0035 | 0.0035 (base) |
| `dbs_burst_ms` | 60 | 60 |
| `min_buffer_size` | 192 | 192 |
| Display | ship PNG **roll-10** mean (paper-silent); gates on raw | same |

Episode-indexed logit nudges apply to the **Baseline** branch only. SEA-DBS uses standard `paper` hyperparameters without those nudges. **v90** reuses cached SEA series and retrains baseline only.

**Prior ship assets (2026-08-10, v89 train → v58 PNG):**

| Asset | Path |
|-------|------|
| PNG | `figures/ravivarapu/images/4a/training_psd_v58.png` |
| Series cache | `artifacts/figures/papers/ravivarapu/4/series.json` (backed up as `series_v89.json` before v90) |
| Checkpoints | `artifacts/figures/papers/ravivarapu/4/baseline_train0.pt`, `paper_train0.pt` |
| Train log | `logs/ravi-fig4a-v89.log` |

**Status:** **Pass** at rep **v59** (`fig4_ravivarapu_config_v90`, display roll10). Gates still green. Mid/late baseline only moved slightly vs v89 (late mean 0.417→0.416; still ~+0.046 vs paper ~0.369) — v90 stim-bridge knobs were too weak for the absolute-level gap. Roll-10 closes roughness visually; level debt remains.

```bash
uv run python -m rl_adaptive_dbs.run scripts/figures/papers/ravivarapu/4a/plot.py --variants baseline --push-kb --update-report
