...
## Locked replication (v108 → rep v67, **Pass**)
...
| `actor_no_stim_bias` | **1.38** | **1.70** (v108) |
| Gumbel-Softmax | forced (`force_gumbel_softmax=True`) | native (`variant=paper`) |
| `gs_lambda` | `6.1e-5` | `1.25e-5` |
| Mid stim logit boost | ep 3–80, **+0.76** | ep 3–70, **+0.15** |
| Midlate stim logit boost | ep 80–150, **+0.65** | none (`0.0`) |
| Late no-stim brake | none | ep 95–150, **+2.25** ramp (v108) |
| `dbs_burst_ms` | **58.0** (8 pulses @ 130 Hz) | **62.0** (9 pulses @ 130 Hz) |
| Display | ship PNG **roll-10** mean (paper-silent); gates on raw | same |

**v108 (rep v67):** `shape_pass` and full `pass` (23/23 gates pass). Baseline matches paper ($0.448 \to 0.407 \to 0.363$ vs paper $0.445 \to 0.411 \to 0.368$). SEA-DBS midlate window (ep 70–110) matches paper ($0.368$ vs paper $0.376$, midlate gap $0.0236$ vs paper $0.0211$), and ramped late brake ($+2.25$ over ep 95–150) softly lands late PSD at $0.3356$ vs paper $0.3400$. Late gap **0.0270** matches paper **0.0279** ($|\Delta| < 0.001$). Pearson B 0.853 / S 0.902. All 23 gates PASS.
