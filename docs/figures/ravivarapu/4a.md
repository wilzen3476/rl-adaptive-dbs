...
`scripts/figures/papers/paper_overlay.py` — Baseline black dashed, SEA-DBS grey
dotted (same source as digitization gates).
...
## Locked replication (v89 train → v42 PNG, seed 0)
...
**Ship assets (2026-08-08):**

| Asset | Path |
|-------|------|
| PNG | `figures/ravivarapu/images/4a/training_psd_v42.png` |
...
**Status:** **Pass** — `shape_pass=True`, `pass=True` (all manifest gates true at v42). Ep 2 SEA PSD **0.456** (v40 dip **0.303** removed).
...
`scripts/probes/ravivarapu_fig4a_series_gap_sweep.py` — piecewise shifts on cached `series.json` to estimate which baseline window deltas pass `dig_gap_widens_mid_to_late` without a 30‑min retrain. Insight: late-only boosts pass gap but fail Pearson; **lowering `gap_midlate` (ep 80–120)** was the winning family.
...
**Probe:** `scripts/probes/ravivarapu_fig4a_fixed_seed_probe.py`
...
Artifacts: `artifacts/probes/ravivarapu_fig4a_fixed_seed_ep5.json`, `ravivarapu_fig4a_fixed_seed_full150_fixed_only.json`.

**Open (not blocking ship):** hybrid reset (continuous plant state across episode boundaries, or fixed seed early only) — needs spec + probe before any convention change.
...
**Probe:** `scripts/probes/ravivarapu_fig4a_smoothness_probe.py` — overlays raw v42 series, rolling means (w=5/10/15), and paper digitization. Artifacts: `artifacts/probes/ravivarapu_fig4a_smoothness_probe.{png,json}`.
