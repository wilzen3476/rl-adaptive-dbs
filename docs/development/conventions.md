...
1. **Exit criteria first** — list qualitative gates in [figures/mehregan/replications.md](../figures/mehregan/replications.md) (or the panel section) before large sweeps.
2. **Ship surface** — committed train+plot entry points live under `scripts/figures/papers/<paper>/<panel>/plot.py` (indexed in [figures/mehregan/replications.md](../figures/mehregan/replications.md)). Shared helpers: `scripts/figures/papers/promote.py`, `scripts/lib/` when imported by panel scripts.
