# Kumaravelu et al. (2016) vendor patches

Upstream: [ModelDB 206232](https://github.com/ModelDBRepository/206232). Citation and provenance: [`readme.txt`](../../reference-material/KumaraveluEtAl2016/readme.txt).

**rl-adaptive-dbs** keeps upstream **network dynamics** in [`reference-material/KumaraveluEtAl2016/`](../../reference-material/KumaraveluEtAl2016/). The edits below are **integration patches only** — base MATLAB, the Python `matlab.engine` bridge, and optional MATLAB-side validation helpers. Do not merge back to ModelDB without review.
...
**Why:** Phase 2 calls network dynamics only; Mehregan $P_\beta$ (13–35 Hz) is computed in Python ([`plant.md`](../plant.md)). `bash scripts/matlab/verify.sh` uses `simulate_network_model(1,1,0,1,true)` (no return values).
...
**Where:** [`plant_band_power.m`](../../reference-material/KumaraveluEtAl2016/plant_band_power.m) — exported `plant_band_power` / `plant_p_beta` plus inlined Chronux `mtspectrumpt` helpers (copied from `simulate_network_model.m`).
